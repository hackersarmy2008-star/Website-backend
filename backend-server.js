...
// serve static (for local dev convenience)
...
app.post('/api/auth/admin-login', async (req, res) => {
  ...
  const token = jwt.sign({ userId: 0, phone: 'admin', isAdmin: true }, JWT_SECRET, { expiresIn: '24h' });
  res.json({ message: 'Admin login successful', token, user: { id: 0, phone: 'admin', isAdmin: true } });
});
...
app.post('/api/user/checkin', authenticateToken, async (req, res) => {
  ...
  try {
    const existing = db.prepare('SELECT id FROM checkins WHERE user_id = ? AND checkin_date = ?').get(userId, today);
    ...
    const checkinTransaction = db.transaction(() => {
      db.prepare('INSERT INTO checkins (user_id, amount, checkin_date) VALUES (?, ?, ?)').run(userId, bonus, today);
      db.prepare('UPDATE users SET balance = balance + ?, total_welfare = total_welfare + ? WHERE id = ?').run(bonus, bonus, userId);
    });
    checkinTransaction();
    const userResult = db.prepare('SELECT balance FROM users WHERE id = ?').get(userId);
    res.json({ message: 'Check-in successful!', bonus, balance: userResult.balance });
  } catch (error) {
...
app.get('/api/admin/pending-withdrawals', authenticateToken, authenticateAdmin, async (_req, res) => {
  try {
...
// non-API fallback to login.html (for local dev)
app.use((req, res, next) => {
...
startServer();  const ADMIN_USERNAME = process.env.ADMIN_USERNAME;
  const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD;
